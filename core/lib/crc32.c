...
/* crc32 hash */
/* not standared implementaion, only for ota upgrade */
unsigned int crc32_data(const unsigned char *data, int len, unsigned int acc)
{
    int i;
    uint32_t crc32val = acc;

    for (i = 0;  i < len;  i++) {
        crc32val = crc32_tab[(crc32val ^ data[i]) & 0xFF] ^ ((crc32val >> 8) & 0x00FFFFFF);
    }
